    ...
    def __init__(self, raw_buff):
        ...
        if self.header.size > self.buff_size:
            raise ResParserError(
                "The file seems to be truncated. Refuse to parse the file! Filesize: {}, declared size: {}".format(self.buff_size,
                                                                                                                   self.header.size))
...
class ARSCHeader:
    ...
    def __init__(self, buff, expected_type=None):
        ...
        if expected_type and self._type != expected_type:
            raise ResParserError(
                "Header type is not equal the expected type: Got 0x{:04x}, wanted 0x{:04x}".format(self._type, expected_type))
